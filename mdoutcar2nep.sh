...
startind=1
vir_on_off=`cat virial.tmp|wc -l`
...
if [ ${vir_on_off} -gt 0 ]
then
echo "$nions 1"|awk -F " " '{print $1,$2}' >> ./Each-NEP/data-${i}.nep
fi
if [ ${vir_on_off} -eq 0 ]
then
echo "$nions 0"|awk -F " " '{print $1,$2}' >> ./Each-NEP/data-${i}.nep
fi	
...
#ennew=`echo "$ener+381"|bc -l`
viri=`sed -n ${i}p virial.tmp`
#ennormvirial=`echo "$ennew $viri"`
enpvirial=`echo "$ener $viri"`
echo "$enpvirial" >> ./Each-NEP/data-${i}.nep
echo "$latt" >> ./Each-NEP/data-${i}.nep
...
cat eachf.tmp >> ./Each-NEP/data-${i}.nep
}
rm latvec.txt
rm testlattxyz.tmp
rm *.tmp
